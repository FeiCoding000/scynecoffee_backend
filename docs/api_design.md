...
Admin activation codes are not generated by this endpoint. Admin accounts are managed manually by backend/development operations.
...
- `role` is optional, defaults to `STAFF`.
- `role` can be `STAFF` or `BARISTA` only. `ADMIN` is rejected.
...
- `400 Bad Request` when `role` is `ADMIN`.
